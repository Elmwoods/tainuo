...
class PubAction extends PublicAction {
    ...
    public function iwaiter() {
        ...
        $waiter = M('item')->where($where)->join("left join ni8_waiter on ni8_waiter.id = ni8_item.waiter_id")->field("ni8_item.*,ni8_waiter.username,ni8_waiter.moble")->order('id desc')->select();
        ob_end_clean();
        $this->ajaxReturn($waiter);
    }
//会员签到
