...
class PubAction extends PublicAction {
	...
	public function pro(){
		...
		foreach($kzlist as $key=>$c)
		{
			...
			if(strpos($kzlink1,"|".$c1['classid']."|")>-1)$msg22.='&nbsp;<input type="checkbox" id="kzlink[]" value="'.$c1['classid'].'" name="kzlink[]">'.$c1['class_name_cn'].'&nbsp;';
			}
		...
		foreach($cslist as $key=>$c)
		{
			...
			$msg3.='<ul style="padding:0px;" class="Father_Title"><li contentEditable="true" rel="'.$c['class_name_cn'].'" name="'.$c['classid'].'">'.$c['class_name_cn'].'</li></ul><ul class="Father_Item'.$i.'">'.$msg33.'</ul><div style="clear:both; height:1px;"></div>';	$i++;
		}
...
public function xian(){
	...
	if($show['prv_id']>0)$show=$this->finds($data[0],'classid='.$show['all_id'],'sort desc,classid desc');
	if($show){
