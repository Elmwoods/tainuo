...
class SystAction extends PublicAction {
	...
	public function tjj(){
		...
		$tj4=$this->tj('dd','qy<2');
		$tj5=$this->tj('tcdd','passed=1');
		$tj6=$this->tj('ddp','isth>0');
		$tj7=$this->tj('gwc','id>0');
		$tj8=$this->tj('message','qy=0');
		$tj9=$this->tj('pro','passed=1');
		$tj10=$this->tj('pro','passed=0');
		$tj11=$this->tj('rebates','is_pay=1 and money>0');
		$tj12=$this->tj('scj','1');
		$tj13=$this->tj('user','qy=0');
		$tj14=$this->tj('user','qy=1');
		$tj15=$this->tj('yhk','1');
		$tj16=$this->tj('fwyhqlq','1');
		$tj17=$this->tj('yhtx','1');
		...
		array("title"=>"套餐订单","sl"=>$tj5),
		array("title"=>"售后订单","sl"=>$tj6),
		array("title"=>"临时购物车","sl"=>$tj7),
		array("title"=>"产品评论","sl"=>$tj8),
		array("title"=>"上架产品","sl"=>$tj9),
		array("title"=>"下价产品","sl"=>$tj10),
		//array("title"=>"返利订单","sl"=>$tj11),
		array("title"=>"收藏夹","sl"=>$tj12),
		array("title"=>"PC会员","sl"=>$tj13),
		array("title"=>"微信会员","sl"=>$tj14),
		//array("title"=>"银行卡","sl"=>$tj15),
		array("title"=>"领取拉动券","sl"=>$tj16),
