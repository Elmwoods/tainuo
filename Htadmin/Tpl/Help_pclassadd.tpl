	...
	<input name="qy" type="hidden" value="<{:cookie('qy')}>" />
		...
		<tr>
			...
			<if condition="($ispic eq 1)">
			 <tr>
    <th>&nbsp;</th>
    <td>*请上传图为126px × 126px的图片 500K以内</td>
  </tr>
			<tr>
                <th>图片：</th>
                <td><img id="thumbshow_1" width="100" height="100" src="<{$show[class_img]|default='no'}>" onerror="javascript:this.src='<{:C('web_url')}>__WJ__/images/n.jpg';"><input name="class_img" type="hidden" id="pic_1"  size="53" class="cd" value="<{$show.class_img}>"/><input type="button" id="image1" value="选择图片" /><if condition="($show.class_img  neq '')"><a href="<{:C('web_url')}>__APP__/home_delfc.html?classid=<{$show.classid}>&kk=typepnews&filed=class_img">删除文件</a></if>
                </td>
            </tr>
			</if>
			<if condition="$Think.cookie.qy eq 2">
			<tr>
                <th>介绍：</th>
                <td>
                    <textarea name="class_txt_cn" cols="60" rows="5" class="cdtext" id="class_txt_cn"><{$show.class_txt_cn}></textarea>                </td>
            </tr>
			</if>
			<!--<tr>
                <th>优化标题：</th>
