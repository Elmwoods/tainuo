...
<div class="mainbox topborder">
    ...
    <td>*请上传图为100px × 100px的图片</td>
			...
			<tr>
                <th>图片：</th>
                <td><img id="thumbshow_1" width="100" height="100" src="<{$show[class_img]|default='no'}>" onerror="javascript:this.src='<{:C('web_url')}>__WJ__/images/n.jpg';"><input name="class_img" rel="no" type="hidden" id="pic_1" size="53" class="cd" value="<{$show.class_img}>"/><input type="button" id="image1" value="选择图片" /><if condition="($show.class_img  neq '')"><a href="<{:C('web_url')}>__APP__/home_delfc.html?classid=<{$show.classid}>&kk=Typeprokz&filed=class_img">删除文件</a></if>
                </td>
            </tr>
			</if>
			<if condition="($dj neq 1 && $dj neq 2)">
			<tr>
                <th>优化标题：</th>
                <td>
			...
			<if condition="($dj eq 1 && $Think.cookie.qy eq 200)">
			<tr>
                <th>是否传图：</th>
                <td><input type="radio" name="ispic" value="1" <if condition="($show[ispic] eq 1)">checked=""</if> >
是
<input type="radio" name="ispic" value="0" <if condition="($show[ispic] eq 0)">checked=""</if>>
否 </td>
            </tr>
			</if>
