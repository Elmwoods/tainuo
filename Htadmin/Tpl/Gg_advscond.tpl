...
<div class="mainbox topborder">
	...
	<input name="id_not" type="hidden" value="<{$show.id}>" />		
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="1" class="fromtj">
			...
			<tr>
                <th>广告位：</th>
                <td><select name="group_id" style="width:auto;" id="group_id" class="cdselect">
  <option value="0">选择位置</option>
 <volist name="farr" id="vol">
	  <option value="<{$vol.id}>" <if condition="($show[group_id] eq $vol[id])">selected="selected"</if>><{$vol.fz}>-<{$vol.name}></option>
	  </volist>
</select></td>
            </tr>
			<tr>
			...
			<tr id="xspic" <if condition="($show[type] eq 1)">style="display:none;"</if>>
                <th>图片：</th>
                <td><img id="thumbshow_1" width="160"  src="<{$show[spic]|default='no'}>" onerror="javascript:this.src='<{:C('web_url')}>__APP__/pub_nopic.html?l=160&w=160';"><input name="spic" type="hidden" id="pic_1" rel="no" size="53" class="cd" value="<{$show.spic}>"/><input type="button" id="image1" value="选择图片" /><if condition="($show.spic  neq '')"><a href="<{:C('web_url')}>__APP__/home_delf.html?id=<{$show.id}>&kk=Advs_con&filed=spic">删除文件</a></if>
                </td>
            </tr>			
