...
function addinfo(act,ids) {
               if (ids == null || ids ==''){
               parent.MsgBox.OpenWin({ id: 'MenuEdit', title: '添加过滤词', width: 1000, height: 400, url: urlS+'syst_keywordd.html?act='+act});
			   }
			   else
			   {
			    parent.MsgBox.OpenWin({ id: 'MenuEdit', title: '修改过滤词', width: 1000, height: 400, url: urlS+'syst_keywordd.html?act='+act+'&id='+ids});
			   }
        }
		</script>
</head>
...
<!-- 新窗口的选项卡结束 -->
    <div class="tit">
        <div class="icontithome">过滤词管理</div>
	...
	<button type="button" class="btn btn-primary btn-sm" onclick="addinfo('add','');">添加过滤词</button>
	...
	<div class="search"><form id="searchform" action="" method="get"><span>词组：</span><span><input name="title" type="text" class="cd" value="<{$title}>"/>&nbsp;</span>
	...
	<table width="100%" class="table table-bordered table-hover">
				<tr>
			      ...
			      <tr>
						...
						<td><{$vo.id}></td>						
						<td><{$vo.keyword}></td>
						<td><{$vo.replace|default='*'}></td>
						<td>替换</td>
						<td><{$vo[time]|date='Y-m-d H:i:s',###}></td>
						<td><if condition="$vo[statu] eq 1">是<else/>否</if></td>
						<td>
						<span onclick="addinfo('edit','<{$vo.id}>');">[<a href="javascript://">修改</a>]</span>&nbsp;&nbsp;&nbsp;
						<span>[<a href="<{:C('web_url')}>__APP__/syst_keyword.html?id=<{$vo.id}>" onClick="return ConfirmDel();">删除</a>]</span></td>
				  </tr>
				  </volist>
		</table>
	  </form>	     
