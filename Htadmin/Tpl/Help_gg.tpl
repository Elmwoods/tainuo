...
function addinfo(act,ids) {
               if (ids == null || ids ==''){
               parent.MsgBox.OpenWin({ id: 'MenuEdit', title: '添加<{$tit}>', width: 1000, height: 600, url: urlS+'help_ggd.html?act='+act});
			   }
			   else
			   {
			    parent.MsgBox.OpenWin({ id: 'MenuEdit', title: '修改<{$tit}>', width: 1000, height: 600, url: urlS+'help_ggd.html?act='+act+'&id='+ids});
			   }
        }
		</script>
</head>
...
<!-- 新窗口的选项卡结束 -->
    <div class="tit">
        <div class="icontithome"><{$tit}>管理</div>
	...
	<button type="button" class="btn btn-primary btn-sm" onclick="addinfo('add','');">添加<{$tit}></button>
	...
	<table width="100%" class="table table-bordered table-hover">
				<tr>
			      <th width="21"><input name="allbox" type="checkbox" id="allbox" onClick="CA();" value="Check All"></th>
					<th width="75">ID</th>					
					<th width="300">信息标题<if condition="($Think.cookie.qy eq 5)">链接地址</if></th>					
			      ...
			      <tr>
				        <td><if condition="($Think.cookie.qy eq 2 && $vo[id] neq 5)"><input name='articleid[]' type='checkbox' onClick="unselectall()" id="articleid" value='<{$vo.id}>'></if></td>
						<td><{$vo.id}></td>						
						<td><{$vo.title}><br />
<if condition="($Think.cookie.qy eq 5)"><{:C('web_url')}>__ROOT__/why_gg?id=<{$vo.id}></if></td>						
						<td><{$vo.addtime}></td>
						<td><{$vo.sort}></td>
						<td><if condition="$vo.passed eq 0">已禁止<else/>已发布</if></td>
						<td>
						<span onclick="addinfo('edit','<{$vo.id}>');">[<a href="javascript://">修改</a>]</span>&nbsp;&nbsp;&nbsp;
						<if condition="($Think.cookie.qy eq 2 && $vo[id] neq 5)"><span>[<a href="<{:C('web_url')}>__APP__/help_gg.html?id=<{$vo.id}>" onClick="return ConfirmDel();">删除</a>]</span></if></td>
				  </tr>
				  </volist>
		</table>
	  </form>	     
