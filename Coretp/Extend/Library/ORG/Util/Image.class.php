...
class Image {
    ...
    static function nopp($text, $type = 'png', $width, $height) {
        ...
        $leng = strlen($text);
        $backColor = imagecolorallocate($im, 248, 246, 246);    //背景色（随机）
        $borderColor = imagecolorallocate($im, 248, 246, 246);                    //边框色
        imagefilledrectangle($im, 0, 0, $width - 1, $height - 1, $backColor);
        imagerectangle($im, 0, 0, $width - 1, $height - 1, $borderColor);
        $stringColor = imagecolorallocate($im, 135, 135, 135);
        imagestring($im, 5, $width / 2 - $leng / 2 * 9, $height / 2 - 5, $text, $stringColor);
    ...
    static function buildImageVerify($length = 4, $mode = 1, $type = 'png', $width = 48, $height = 22, $verifyName = 'verify') {
        ...
        //setcookie($verifyName."s",$randval,0,'/','',false,true);
        error_log($randval.PHP_EOL, 3, 'sss.log');
        setcookie($verifyName, md5($randval), 0, '/', '', false, true);
    }
