...
KindEditor.plugin('filemanager', function(K) {
	...
	self.plugin.filemanagerDialog = function(options) {
		...
		function createView(result) {
			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				var data = fileList[i],
					//gai//div = K('<div class="ke-inline-block ke-item"></div>');
...
//					});
				...
				var _span=K('<span class="xl_span" data-url="'+iconUrl+'" style="position:absolute;display:none;width:102px;background:#0690d2;color:#FFF;text-align:center; cursor: pointer;line-height:20px;bottom:-3px;left:0;">删除</span>');
				div.append(_span);
			    }
			}
			//
			$(".xl_span").click(function(){//这里的.xl_span对应着刚才插入的删除按钮上的class
				...
				var lj=self.pluginsPath;	
				lj=lj.replace("webfile/admin/ke/plugins/",'htni8.php/');				
				lj=lj+"home_filedel.html";
				$.post(lj,{action:"delss",url:$this.attr("data-url")},function(res){//jquery的post，action为指定的配合后端用，url是获取刚才存在删除按钮上的图片路径，你完全可以用别的来写，因为用不好kindeditor的js库才用的jquery
