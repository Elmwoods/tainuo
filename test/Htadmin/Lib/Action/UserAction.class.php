...
//            if (!is_numeric($username) || !preg_match("/1[345678]{1}\d{9}$/", $username)) {
//                $this->assign('sx', 1);
//                $this->display("pub:load");
//                exit;
//            }
...
//            }
    ...
    public function level(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $allid = $_POST['articleid'];
            foreach ($allid as $key => $id) {
                $show = $this->finds("level", 'id=' . $id . '', 'id desc');
                $this->del('level', 'id=' . $show['id']);
            }
            echo "<script language='javascript'>location='" . $this->ly . "';</script>";
            exit;
        }
        ...
        if (!empty($_GET['id'])) {
            $id = (int) $_GET['id'];
            $show = $this->finds("level", 'id=' . $id . '', 'id desc');
            $a = $this->del('level', 'id=' . $show['id'] .'');
            echo "<script language='javascript'>location='" . $this->ly . "';</script>";
            exit;
        }

        $show = $this->joinarr('level', 20, '', '', 'id desc');
        $this->assign('show', $show['list']);
        $this->assign('count', $show['count']);
        $this->assign('fpage', $show['cpage']);
        $this->display();
    }
    ...
    public function levelinfo(){
        if ($_SERVER['REQUEST_METHOD'] == 'GET'){
            $show = $this->finds('level', 'id = '. $_GET['id'].'', 'id desc');
            $this->assign('show', $show);
        }

        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if($_POST['level_name'] == '' || $_POST['price'] == '' ){
                return false;
            }
            if((int)$_POST['id_not']){
                $save = $this->save('level', array("level_name" => $_POST['level_name'], "price" => $_POST['price'], "utime" => time()), 'id ='.$_POST['id_not'] );
                if($save){
                    $this->assign('sx', 1);
                    $this->display("pub:load");
                    exit;
                ...
                $add_level_info = $this->add("level", 
                    array("level_name" => $_POST['level_name'], "price" => $_POST['price'], "ctime" => time()
                ));
                if($add_level_info){
                    $this->assign('sx', 1);
                    $this->display("pub:load");
