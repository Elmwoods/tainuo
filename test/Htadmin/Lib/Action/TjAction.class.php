...
function tx_data(){
        ...
        $sql   = 'SELECT COUNT(*) a1,SUM(price) a2,SUM(case when `passed` = 0 then `price` end) a3,SUM(case when `passed` = 1 then `price` end) a4,SUM(case when `passed` = 2 then `price` end) a5,SUM(case when `passed` = 3 then `price` end) a6 FROM `ni88_yhtx` '.$where;
		
        $run_data = M("")->query($sql);  
		
		$sql   = 'SELECT SUM(discount) a7,SUM(point) a8,SUM(pointend) a9 FROM `ni88_user` ';
...
function order_data(){
        ...
        $sql   = 'SELECT COUNT(*) max,COUNT(case when `passed` = 0 then `passed` end) npassed,COUNT(case when `passed` >0 and `passed` <>4  then `passed` end) ypassed,SUM(case when `passed` = 0 then `zhprice` end) price,SUM(case when `passed` >0 and `passed` <>4 then `zhprice` end) yprice FROM `ni88_dd` '.$where;
        $run_data = M("")->query($sql);
        ...
        $sql   = 'SELECT SUM(`tk`.price) tkprice,SUM(case when `tk`.isth = 1 then `tk`.price end) tkprice1,SUM(case when `tk`.isth = 2 then `tk`.price end) tkprice2,COUNT(case when `tk`.isth = 3 then `tk`.isth end) tkprice3,COUNT(case when `tk`.isth = 2 then `tk`.isth end) tkprice4,COUNT(case when `tk`.isth = 1 then `tk`.isth end) tkprice5,COUNT(case when `ddp`.ispassed <> 5 and `ddp`.ispassed <> 6 then `tk`.isth end) cl1,COUNT(case when `ddp`.ispassed = 5  then `tk`.isth end) cl2,COUNT(case when `ddp`.ispassed = 6  then `tk`.isth end) cl3 FROM `ni88_tk` tk LEFT JOIN `ni88_ddp` ddp ON `tk`.ddpid = `ddp`.id '.$where;
        $out_data = M("")->query($sql);
...
function cz_data(){
        ...
        $sql   = "SELECT COUNT(*) max,COUNT(case when `sz` not like '%后台%' then `passed` end) qt,COUNT(case when `sz` like '%后台%' then `passed` end) ht,SUM(case when `sz` not like '%后台%' then `price` end) qprice,SUM(case when `sz` like '%后台%' then `price` end) hprice,SUM(price) aprice FROM `ni88_pjl` ".$where;
        $run_data = M("")->query($sql);    
        return $run_data[0];
...
function yj_data(){
        ...
        $sql   = 'SELECT COUNT(*) max,COUNT(case when `passed` = 0 then `passed` end) npassed,COUNT(case when `passed` = 1 then `passed` end) ypassed,SUM(case when `passed` = 0 then `price` end) price,SUM(case when `passed` = 1 then `price` end) yprice,SUM(price) aprice FROM `ni88_jl` '.$where;
        $run_data = M("")->query($sql);

        $sql   = 'SELECT SUM(balances) balances,SUM(balancesend) balancesend,SUM(balancesends) balancesends FROM `ni88_user` ';
        $out_data = M("")->query($sql);
        $aa=array();
		if($run_data[0]){
		 $aa=array_merge($aa,$run_data[0]);
...
function user_data(){
        ...
        //一条找出全部 包括跨表
        $sql = 'SELECT COUNT(*) max,COUNT(case when `user`.vip=0 then `user`.vip end) vip1,COUNT(case when `user`.vip = 6 then `user`.vip end) vip2,COUNT(case when `user`.vip = 7 then `user`.vip end) vip3,COUNT(case when `user`.vip = 8 then `user`.vip end) vip4,COUNT(case when `verify`.passed = 0 then `verify`.passed end) ver_passed,COUNT(case when `verify`.passed = 1 then `verify`.passed end) ver_passed1,COUNT(case when `verify`.passed = 2 then `verify`.passed end) ver_passed2 FROM `ni88_user` user LEFT JOIN `ni88_verify` verify ON `user`.id = `verify`.user_id '.$where;
        return M("")->query($sql);
...
function fp_data(){
        ...
        $sql = 'SELECT SUM(case when `dd`.passed >0 and `dd`.passed <>4 then `zhprice` end) p1 FROM `ni88_fp` fp LEFT JOIN `ni88_dd` dd ON `fp`.ddid = `dd`.classid where `fp`.ddid>0';		
        $run_data = M("")->query($sql);    
        return $run_data[0];
...
function yhq_data(){
        ...
        $sql = 'SELECT SUM(jg) yh1,SUM(case when `yhqlq`.passed=1 then `jg` end) yh2,SUM(case when `yhqlq`.passed=0 and etimes<'.time().' then `jg` end) yh3,SUM(case when `yhqlq`.passed=0 and etimes>'.time().' then `jg` end) yh4 FROM `ni88_yhqlq` yhqlq ';		
        $run_data = M("")->query($sql);    
        return $run_data[0];
