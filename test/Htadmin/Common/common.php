...
function cate($date, $id) {
    $title = M($date)->field("title")->where("id = {$id}")->find()['title'];
    return $title ? $title : "无";
}
function receivestatus($val) {
    $arr = array(
        0 => '发送失败',
        1 => '发送成功',
        2 => '已领取',
...
function sumprice($table, $field, $id, $sumfield) {
    $sum = M($table)->where("{$field} = {$id}")->sum($sumfield);
    return $sum ? $sum : 0;
}
...
function links($link_id, $date) {
    ...
    $news = M($date);
    if ($link_id[1])
        $region_name = $news->cache(true, 0)->where('classid=' . $link_id[1])->order('classid desc')->getField('class_name_cn');
    if ($link_id[2]) {
