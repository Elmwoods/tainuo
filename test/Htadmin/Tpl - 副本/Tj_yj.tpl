...
<!-- 新窗口的选项卡结束 -->
    <div class="tit">
        <div class="icontithome">拥金提现详细</div>
...
<div class="search" style="background-color:#ececec; height:50px; padding-top:10px;">&nbsp;
  <input type="button" value="搜索/筛选数据" onclick="dosearch();" class="btn btn-primary  search_action"/><input type="button" value="将当前条件下数据导出为Excel" class="btn btn-primary" onclick="location.href='<{:C('web_url')}>__APP__/tj_yjexport';"/>
</div>
	<div class="clear"></div>
	<!--内容-->
	...
	<table width="100%" class="table table-bordered">
				<tr>
			      ...
			      <tr>
						...
						<td><{$vo.price}></td>
						<td><if condition="$vo['passed'] eq 0">待审核<else/>已审核</if></td>
						<td><{$vo.addtime}></td>
						<td><{$vo.cltime}></td>
						<td><{$vo.clr}></td>
						<td><{$vo.sz}></td>
