...
<form id="myform" name="myform" method="post" action="">
  ...
  <div id="aEditor-Body">
	...
	<option value="0" selected>请选择分类</option>
	 <volist name="lb" id="vol">
	 <optgroup label="<{$vol.class_name_cn}>"></optgroup>
	 <volist name="vol[0]" id="vol1">
 <option <if condition="($vol1[classid] eq $show[classid])">selected="selected"</if> value="<{$vol1.classid}>">─<{$vol1.class_name_cn}></option>
</volist>
</volist>
</select>
						</td>
				...
				<tr>
    <th>&nbsp;</th>
    <td>*请上传图为500px × 500px的图片 500K以内</td>
  </tr>
			<tr>
                <th>图片：</th>
                <td><img id="thumbshow_1" width="160" height="125" src="<{$show[spic]|default='no'}>" onerror="javascript:this.src='<{:C('web_url')}>__WJ__/images/n.jpg';"><input name="spic" type="hidden" id="pic_1" size="53" class="cd" value="<{$show.spic}>"/><input type="button" id="image1" value="选择图片" /><if condition="($show.spic  neq '')"><a href="<{:C('web_url')}>__APP__/home_delf.html?id=<{$show.id}>&kk=Help&filed=spic">删除文件</a></if>
                </td>
            </tr>
                ...
                <tr>
				 ...
				 <tr>
                     ...
                     <td><input type="text" name="hits" id="hits"  value="<{$show.hits|default='0'}>" class="cd50"><font color="red">数字越大越靠前</font></td>
                </tr>
				 <tr>
                     <th><font color="red">*</font>排序：</th>
                     <td><input type="text" name="sort" id="sort"  value="<{$show.sort|default='0'}>" class="cd50"><font color="red">数字越大越靠前</font></td>
                </tr>
				 <tr>
                     <th><font color="red">*</font>发布时间：</th>
                     <td><input type="text" name="addtime_time" id="addtime"  value="<{$show.addtime|default=$time}>" class="cd150" onClick="WdatePicker({doubleCalendar:true,dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'%y-%M-{%d+0} HH:mm:ss',maxDate:'%y-{%M+3}-{%d-1} HH:mm:ss'})" readonly></td>
                </tr>
			</tbody></table>
		</div>
	</div>
    ...
    <div class="tableform">
		<div class="division">
			<table cellspacing="0" cellpadding="0" border="0">
				...
				<tr>
					<th>SEO 标题：</th>
					<td><input type="text" vtype="text" id="t" name="t" class="x-input cd300 " autocomplete="off" value="<{$show.t}>"></td>
				</tr>				
				<tr>
					<th>SEO 关键字：</th>
					<td><input type="text" vtype="text" id="k" name="k" class="x-input cd300" autocomplete="off" value="<{$show.k}>"></td>
				</tr>				
				<tr>
					<th>SEO 描述：</th>
					<td><textarea name="d" cols="60" rows="5" class="x-input cdtext" id="d" vtype="text" autocomplete="off"><{$show.d}></textarea></td>
				</tr>				
			</tbody></table>
        </div>
	</div>
