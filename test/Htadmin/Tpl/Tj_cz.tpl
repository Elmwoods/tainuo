...
<!-- 新窗口的选项卡结束 -->
    <div class="tit">
        <div class="icontithome">充值记录详细</div>
...
<option <if condition="($passed eq '0')">selected="selected"</if>  value="0">前端</option>
<option <if condition="($passed eq '1')">selected="selected"</if>  value="1">后端</option>
...
<div class="search" style="background-color:#ececec; height:50px; padding-top:10px;">&nbsp;
  <input type="button" value="搜索/筛选数据" onclick="dosearch();" class="btn btn-primary  search_action"/><input type="button" value="将当前条件下数据导出为Excel" class="btn btn-primary" onclick="location.href='<{:C('web_url')}>__APP__/tj_czexport';"/>
</div>
	<div class="clear"></div>
	<!--内容-->
	...
	<table width="100%" class="table table-bordered">
				<tr>
			      <th width="22"><input name="allbox" type="checkbox" id="allbox" onClick="CA();" value="Check All"></th>
					...
					<th width="150">钱包余额</th>
					<th width="60">订单号</th>
					<th width="240">提交时间/支付时间</th>
					<th width="60">充值状态</th>
                    <th width="50">说明</th>									
