	...
	<input name="id_not" type="hidden" value="<{$show.id}>" />	
		...
		<tr>
			...
			<tr>
				...
				<tr>
				  <th>帐户余额：</th>
				  <td><if condition="$show eq ''">
				  <input name="discount" type="text" id="discount" class="cd150"  value="<{$show.discount|default='0'}>"/>
				  <else/><{$show.discount}>&nbsp;&nbsp;<a href="<{:C('web_url')}>__APP__/user_recharge?userid=<{$show.id}>">人工充值通道</a></if>
                                  <span class="red">余额不足无法发放工资</span></td>
		  </tr>		
			<tr>
                <th>头像：</th>
