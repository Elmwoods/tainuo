...
function addinfo(act,ids) {
               if (ids == null || ids ==''){
               parent.MsgBox.OpenWin({ id: 'MenuEdit', title: '添加酒店', width: 1000, height: 500, url: urlS+'pro_hsave.html?act='+act});
			   }
			   else
			   {
			    parent.MsgBox.OpenWin({ id: 'MenuEdit', title: '酒店信息', width: 1000, height: 500, url: urlS+'pro_hsave.html?act='+act+'&id='+ids});
			   }
        }
		</script>
</head>
...
<!-- 新窗口的选项卡结束 -->
    <div class="tit">
        <div class="icontithome">酒店管理列表</div>
	...
	<button type="button" class="btn btn-primary btn-sm" onclick="addinfo('add','');">添加酒店</button>
...
<{$vo.title}></td>
						...
						<span>[<a href="<{:C('web_url')}>__APP__/pro_hotel.html?id=<{$vo.id}>" onClick="return ConfirmDel();">删除</a>]</span>
						</td>
				  </tr>				  
				  </volist>
		</table>
