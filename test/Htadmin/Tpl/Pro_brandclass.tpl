...
<script type="text/javascript">        
        function add(id,dj,prv) {
            if (id == null || id =='')
                parent.MsgBox.OpenWin({ id: 'MenuEdit', title: '添加<{$kzname}>分类', width: 1000, height: 700, url: urlS+'pro_brandclassadd.html?act=add&dj='+dj+'&prv='+prv});
            else
                parent.MsgBox.OpenWin({ id: 'MenuEdit', title: '修改<{$kzname}>分类', width: 1000, height: 700, url: urlS+'pro_brandclassadd.html?act=edit&id='+id+"&dj="+dj+"&prv="+prv});
        }		
...
<!-- 新窗口的选项卡结束 -->
	...
	<div class="search"><form id="searchform" action="" method="get"><span>栏目名称：</span><span><input name="title" type="text" class="cd" value="<{$title}>"/>&nbsp;</span>
	...
	<table width="100%" class="table table-bordered table-hover">
				...
				<volist name="classlist" id="vol">
			      <tr>
						...
						<span class="opt lnk red" onclick="passed('typebrand','<{$vol.classid}>');" id="passed_<{$vol.classid}>">
<img src="<{:C('web_url')}>__WJ__/images/no.gif" app="desktop">已禁用</span>
<else/>
<span class="opt lnk" onclick="passed('typebrand','<{$vol.classid}>');" id="passed_<{$vol.classid}>">
...
<span class="opt lnk" onclick="deleteRow('typebrand',<{$vol.classid}>,'classid','',<{$vol.prv_id}>,'pro_brandclass');">
<img border="0" app="desktop" alt="删除" src="<{:C('web_url')}>__WJ__/images/delecate.gif">
删除
</span>

						</td>
						<td>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" value="<{$vol.tj}>" <if condition="$vol.tj eq 1">checked</if> data="<{$vol.classid}>" title="typebrand" name="tj[]"></td>
				  </tr>
				  ...
				  <tr class="active" id="xj<{$vol.classid}>" <{$link_id|midd=###,$vol[classid],1}>>
						...
						<span class="opt lnk red" onclick="passed('typebrand','<{$vol1.classid}>');" id="passed_<{$vol1.classid}>">
<img src="<{:C('web_url')}>__WJ__/images/no.gif" app="desktop">已禁用</span>
<else/>
<span class="opt lnk" onclick="passed('typebrand','<{$vol1.classid}>');" id="passed_<{$vol1.classid}>">
<img src="<{:C('web_url')}>__WJ__/images/yes.gif" app="desktop">已发布</span>
</if>
</td>
						<td>&nbsp;</td>
...
<span class="opt lnk" onclick="deleteRow('typebrand',<{$vol1.classid}>,'classid','',<{$vol1.prv_id}>,'pro_brandclass');">
<img border="0" app="desktop" alt="删除" src="<{:C('web_url')}>__WJ__/images/delecate.gif">
删除
</span>
						...
						<td>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" value="<{$vol1.tj}>" <if condition="$vol1.tj eq 1">checked</if> data="<{$vol1.classid}>" title="typebrand" name="tj[]"></td>
				  </tr>
				  </volist>
				</volist>
		</table>
