...
<div class="mainbox topborder">
	...
	<input name="id_not" type="hidden" value="<{$show.id}>" />	
		<div class="hyinfo"><b class="text-warning">部门信息</b></div>
		...
		<tr>
                     <th><font color="red">*</font>所属酒店：</th>
                     <td>
                         <select name="hotel_id" id="" class="cdselect">
                             <option value="0">选择酒店</option>
                             <foreach name="hotel" item="v">
                                <option <if condition="($show['hotel_id'] eq $v['id'])">selected="selected"</if>  value="<{$v.id}>"><{$v.title}></option>
                            </foreach>
                         </select>
                     </td>
                </tr>					
