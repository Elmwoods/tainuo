...
class Wxpay_client_pub extends Common_util_pub {
     ...
     */
    function createXml() {
        $this->parameters["appid"] = WxPayConf_pub::$addid; //公众账号ID
 ...
 * 现金红包
 */
class Send_redpack extends Wxpay_client_pub {
    ...
    function createXml() {
        try {
            ...
            if ($this->parameters["mch_billno"] == null) {
                throw new SDKRuntimeException("发送现金红包申请中，缺少必填参数mch_billno！" . "<br>");
            } elseif ($this->parameters["send_name"] == null) {
                throw new SDKRuntimeException("发送现金红包申请中，缺少必填参数send_name！" . "<br>");
            } elseif ($this->parameters["re_openid"] == null) {
                throw new SDKRuntimeException("发送现金红包申请中，缺少必填参数re_openid！" . "<br>");
            } elseif ($this->parameters["total_amount"] == null) {
                throw new SDKRuntimeException("发送现金红包申请中，缺少必填参数total_amount！" . "<br>");
            } elseif ($this->parameters["wishing"] == null) {
                throw new SDKRuntimeException("发送现金红包申请中，缺少必填参数wishing！" . "<br>");
            } elseif ($this->parameters["client_ip"] == null) {
                throw new SDKRuntimeException("发送现金红包申请中，缺少必填参数client_ip！" . "<br>");
            } elseif ($this->parameters["act_name"] == null) {
                throw new SDKRuntimeException("发送现金红包申请中，缺少必填参数act_name！" . "<br>");
            } elseif ($this->parameters["remark"] == null) {
                throw new SDKRuntimeException("发送现金红包申请中，缺少必填参数remark！" . "<br>");
            } elseif ($this->parameters["scene_id"] == null) {
                throw new SDKRuntimeException("发送现金红包申请中，缺少必填参数scene_id！" . "<br>");
            } elseif ($this->parameters["scene_id"] == null) {
                throw new SDKRuntimeException("发送现金红包申请中，缺少必填参数scene_id！" . "<br>");
            }
            $this->parameters["wxappid"] = WxPayConf_pub::$addid; //公众账号ID
            $this->parameters["mch_id"] = WxPayConf_pub::$mchid; //商户号
            $this->parameters["nonce_str"] = $this->createNoncestr(); //随机字符串
 ...
 * 红包记录查询接口
 */
class Send_query extends Wxpay_client_pub {

    function __construct() {
        //设置接口链接
        $this->url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/gethbinfo";
        //设置curl超时时间
        $this->curl_timeout = WxPayConf_pub::CURL_TIMEOUT;
    }
    ...
    function createXml() {
        try {
            ...
            $this->parameters["mch_id"] = WxPayConf_pub::$mchid; //商户号
            $this->parameters["nonce_str"] = $this->createNoncestr(); //随机字符串
            $this->parameters['bill_type'] = 'MCHT';
            $this->parameters["sign"] = $this->getSign($this->parameters); //签名
...
class NativeCall_pub extends Wxpay_server_pub {
    ...
    function createXml() {
        if ($this->returnParameters["return_code"] == "SUCCESS") {
            $this->returnParameters["appid"] = WxPayConf_pub::$addid; //公众账号ID
...
class JsApi_pub extends Common_util_pub {
    ...
    function createOauthUrlForCode($redirectUrl) {
        $urlObj["appid"] = WxPayConf_pub::$addid;
    ...
    public function getParameters() {
        $jsApiObj["appId"] = WxPayConf_pub::$addid;
        $timeStamp = time();
