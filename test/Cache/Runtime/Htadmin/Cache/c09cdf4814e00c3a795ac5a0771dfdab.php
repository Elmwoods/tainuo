    ...
    <body>
        ...
        <div class="mainbox topborder">
            ...
            <div class="search"><form id="searchform" action="" method="get">
                    <span>红包总金额: <?php echo ($sum); ?>&nbsp;</span>
                    <span>领班帐号：</span><span><input name="users" type="text" class="cd" value="<?php echo ($users); ?>"/>&nbsp;</span>
                    <span>服务员账号/手机号码：</span><span><input name="waiter" type="text" class="cd" value="<?php echo ($waiter); ?>"/>&nbsp;</span>
                    <span>项目名称：</span><span><input name="subject" type="text" class="cd" value="<?php echo ($subject); ?>"/>&nbsp;</span>
                    <span>操作时间:<input name="ks" style="height:28px;" type="text" class="cd100" value="<?php echo ($ks); ?>" onClick="WdatePicker({doubleCalendar:true,dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'{%y-50}-%M-{%d+0} %H:%m:%s',maxDate:'%y-{%M}-{%d-1}%H:%m:%s'})" readonly/>-<input name="js" type="text" class="cd100" value="<?php echo ($js); ?>" style="height:28px;" onClick="WdatePicker({doubleCalendar:true,dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'{%y-50}-%M-{%d+0} %H:%m:%s',maxDate:'%y-{%M}-{%d-1}%H:%m:%s'})" readonly/>&nbsp;
                        <span>工作日期:<input name="wks" style="height:28px;" type="text" class="cd100" value="<?php echo ($wks); ?>" onClick="WdatePicker({doubleCalendar:true,dateFmt:'yyyy-MM-dd'})" readonly/>-<input name="wjs" type="text" class="cd100" value="<?php echo ($wjs); ?>" style="height:28px;" onClick="WdatePicker({doubleCalendar:true,dateFmt:'yyyy-MM-dd'})" readonly/>&nbsp;

                            <span><select name="passed" style="width:80px" class='cdselect' >
                                    <option value="">发送状态</option>
                                    <option <?php if(($passed == '0')): ?>selected="selected"<?php endif; ?>  value="0">发送失败</option>
                                    <option <?php if(($passed == '1')): ?>selected="selected"<?php endif; ?>  value="1">发送成功</option>
                                    <option <?php if(($passed == '2')): ?>selected="selected"<?php endif; ?>  value="2">已领取</option>
                                    <option <?php if(($passed == '3')): ?>selected="selected"<?php endif; ?>  value="3">退款中</option>
                                    <option <?php if(($passed == '4')): ?>selected="selected"<?php endif; ?>  value="4">已退款</option>
                                    <!--<option <?php if(($is_pay == '2')): ?>selected="selected"<?php endif; ?>  value="2">已退款</option>-->
                                </select>&nbsp;</span>
                        </span>
                ...
                <form id="nodecatEditor" name="nodecatEditor" method="post" action="">
                    <table width="100%" class="table table-bordered">
                        ...
                        <?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td><?php echo ($vo["waiter"]); ?> <?php echo ($vo["waitermoble"]); ?></td>
                                <td><?php echo ($vo["title"]); ?></td>
                                <td><?php echo (receivestatus($vo["passed"])); ?></td>
                                <td><?php echo ($vo["msg"]); ?></td>
                                <td><span>[<a href="<?php echo C('web_url');?>__APP__/user_jl.html?id=<?php echo ($vo["id"]); ?>" onClick="return ConfirmDel();">删除</a>]</span>						</td>
