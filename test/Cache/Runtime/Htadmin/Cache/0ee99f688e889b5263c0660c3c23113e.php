...
function addinfo(act,ids) {
		if (ids == null || ids ==''){
				parent.MsgBox.OpenWin({ id: 'MenuEdit', title: '添加服务员', width: 1000, height: 500, url: urlS+'user_wsave.html?act='+act});
		}
		else
		{
		parent.MsgBox.OpenWin({ id: 'MenuEdit', title: '服务员信息', width: 1000, height: 500, url: urlS+'user_wsave.html?act='+act+'&id='+ids});
		}
}
		</script>
</head>
...
<!-- 新窗口的选项卡结束 -->
    <div class="tit">
        <div class="icontithome">服务员管理列表</div>
	...
	<button type="button" class="btn btn-primary btn-sm" onclick="addinfo('add','');">添加服务员</button>
				...
				<tr>
			      <th width="22"><input name="allbox" type="checkbox" id="allbox" onClick="CA();" value="Check All"></th>
					...
					<?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo M('level')->where(array('id' => $vo['level']))->find()['level_name']; ?></td>
						...
						<span>[<a href="<?php echo C('web_url');?>__APP__/user_waiter.html?id=<?php echo ($vo["id"]); ?>" onClick="return ConfirmDel();">删除</a>]</span>
						</td>
